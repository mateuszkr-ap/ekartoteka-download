...
class EkartotekaClient:
    ...
    def get_uchwala_file(self, uchwala, active=True):
        ...
        for i, zal in enumerate(zalaczniki):
            base_url = f"https://e-kartoteka.pl/api/uchwaly/uchwaly/{zal['id_uch']}/zalaczniki/{zal['id']}/"
            ...
            download_file(download_url, f"{location.as_posix()}/{name}", headers=self.headers)
        return response.json()
